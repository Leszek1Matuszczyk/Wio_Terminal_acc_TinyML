...
/* Includes ---------------------------------------------------------------- */
#include <Leszek_Akcelerometr_AI_inferencing.h>
#include "LIS3DHTR.h"
#include "TFT_eSPI.h"
...
void loop() {
  ...
  if (result.classification[3].value < 0.7 & result.classification[2].value < 0.7 & result.classification[1].value < 0.7 & result.classification[0].value < 0.7) {
    tft.fillScreen(TFT_WHITE);
    tft.setFreeFont(&FreeSansBoldOblique12pt7b);
    tft.drawString("LOL nie wyszlo", 20, 80);
    delay(100);
    Serial.println("wave");
  }
}
